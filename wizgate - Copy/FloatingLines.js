...
export function initFloatingLines(container, options = {}) {
  const {
    linesGradient = ['#3B82F6', '#10B981', '#F59E0B'],
  ...
  const tCount = enabledWaves.includes('top') ? getLineCount('top') : 0;
  const mCount = enabledWaves.includes('middle') ? getLineCount('middle') : 0;
  const bCount = enabledWaves.includes('bottom') ? getLineCount('bottom') : 0;

  const tDist = enabledWaves.includes('top') ? getLineDistance('top') * 0.01 : 0.01;
  const mDist = enabledWaves.includes('middle') ? getLineDistance('middle') * 0.01 : 0.01;
  const bDist = enabledWaves.includes('bottom') ? getLineDistance('bottom') * 0.01 : 0.01;
  ...
  const uniforms = {
    ...
    topLineCount: { value: tCount },
    middleLineCount: { value: mCount },
    bottomLineCount: { value: bCount },
    topLineDistance: { value: tDist },
    middleLineDistance: { value: mDist },
    bottomLineDistance: { value: bDist },
